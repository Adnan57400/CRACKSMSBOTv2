...
# WhatsApp bridge URLs — override WA_BRIDGE_HOST env var when bridge runs as
# a separate Railway service (e.g. WA_BRIDGE_HOST=https://wa-bridge.railway.app)
_WA_HOST          = os.environ.get("WA_BRIDGE_HOST", "http://127.0.0.1:7891").rstrip("/")
WA_BRIDGE_URL     = f"{_WA_HOST}/control"       # bridge admin control
WA_FORWARD_URL    = f"{_WA_HOST}/forward_otp"   # Python → WA bridge (OTP push)
WA_HEALTH_URL     = f"{_WA_HOST}/health"         # health check endpoint
WA_MEDIA_URL      = f"{_WA_HOST}/forward_media"  # media attachment endpoint
WA_SCHEDULE_URL   = f"{_WA_HOST}/schedule_msg"   # message scheduling
WA_GROUP_URL      = f"{_WA_HOST}/group_action"   # group management
WA_OTP_SECRET     = "cracksms_wa_secret_2026"             # shared secret (keep private)
WA_OTP_PORT       = int(os.environ.get('WA_OTP_PORT', 7890))  # Python WA receive port 
        ...
        for msg in MESSAGE_SCHEDULE[user_id][:]:
            if datetime.now() >= msg["scheduled_time"]:
                try:
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.post(WA_FORWARD_URL, json=payload, timeout=aiohttp.ClientTimeout(total=3)) as resp:
                            if resp.status == 200:
...
async def start_wa_http_server():
    ...
    try:
        ...
        await runner.setup()
        site = web.TCPSite(runner, '0.0.0.0', WA_OTP_PORT)
        await site.start()
